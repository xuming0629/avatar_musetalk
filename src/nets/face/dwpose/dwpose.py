...
# @description   : 人体关键点 / RTMPose ONNX 推理
...
def get_dwpose_path(
    ...
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            f"DWPose / RTMPose ONNX weight not found: {model_path}"
        )
...
def preprocess(
    ...
    for bbox in out_bbox:
        ...
        resized_img = resized_img.astype(
            np.float32,
        )
...
def get_simcc_maximum(
    ...
    simcc_x = simcc_x.reshape(
        N * K,
        -1,
    )

    simcc_y = simcc_y.reshape(
        N * K,
        -1,
    )
...
def postprocess(
    ...
    for i, output in enumerate(outputs):
        ...
        keypoints = (
            keypoints / model_input_size_np * scales[i]
            + centers[i]
            - scales[i] / 2
        )
...
class RTMPose:
    ...
    等价于：
        ...
        rtmpose = RTMPose.from_config("configs/musetalk_v15.yaml")

    也支持直接传 ONNX：

        rtmpose = RTMPose(model_path="models/dwpose/dw-ll_ucoco_384.onnx")
    """
    ...
    def __init__(
        ...
        print(
            f"[RTMPose] load weight success: {self.model_path}"
        )
        print(
            f"[RTMPose] providers: {self.providers}"
        )
