...
def get_face_parse_config(
        config_path="configs/musetalk_v15.yaml",
        project_root=None,
):
    cfg = load_yaml(config_path)

    model_cfg = cfg["models"]["face-parse-bisent"]
...
def get_face_parse_resnet_path(
        config_path="configs/musetalk_v15.yaml",
        project_root=None,
):
    cfg = load_yaml(config_path)
...
class BiSeNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv_out = BiSeNetOutput(
            256,
            256,
            n_classes
        )

        self.conv_out16 = BiSeNetOutput(
            128,
            64,
            n_classes
        )

        self.conv_out32 = BiSeNetOutput(
            128,
            64,
            n_classes
        )
    ...
    def from_config(
        ...
        model_path =get_face_parse_resnet_path(
                config_path,
                project_root,
            )
        n_classes = cfg["n_classes"]
        net = cls(resnet_path=model_path, n_classes=n_classes)
