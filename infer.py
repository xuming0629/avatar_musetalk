...
def run_musetalk_inference(
    ...
    # --------------------------------------------------------
    if progress:
        progress(0.30, desc="检测人脸并编码 latent...")
    ...
    # --------------------------------------------------------
    if progress:
        progress(0.45, desc="执行 MuseTalk 推理...")
    ...
    with torch.no_grad():
        ...
        for batch_start, audio_batch, latent_batch in pbar:
            ...
            if progress:
                cur = min(0.85, 0.45 + 0.40 * (batch_start + args.batch_size) / total_chunks)
                progress(cur, desc="生成视频帧中...")

    # --------------------------------------------------------
    # 5. 帧合成视频
    # --------------------------------------------------------
    if progress:
        progress(0.90, desc="合成无声视频...")
...
def build_demo():
    with gr.Blocks(title="XumingAvatar MuseTalk Demo") as demo:
        gr.Markdown("# XumingAvatar / MuseTalk v1.5 数字人推理 Demo")
        ...
        with gr.Row():
            with gr.Column():
                ...
                # 如果你想支持图片，可以后面单独加 gr.Image(type='filepath')
                # 当前 gr.Video 对部分图片不一定友好，图片建议用 File 输入。
                file_input = gr.File(
                    label="或者上传图片/视频文件",
                    type="filepath",
                )
        ...
        def _infer_wrapper(
            ...
            # 优先使用 file_input，因为它可以兼容图片和视频
            real_video_path = file_path if file_path is not None else video_path
